...
- **自动标题**：首次对话后按用户首条消息自动生成标题，侧栏可原地重命名
...
- **自定义背景图**：上传 / 选择 / 删除当前账号的背景，支持透明度与模糊度调节（≤10MB，jpg/png/webp/gif）
...
- 用户上传存放于 `backend/backgrounds/<user_id>/`，需登录后访问 `/backgrounds/{name}`；也可在 `backend/backgrounds/` 根目录放入共享图片（只读）
